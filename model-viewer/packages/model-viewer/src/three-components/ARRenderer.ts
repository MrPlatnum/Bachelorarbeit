...
export class ARRenderer extends EventDispatcher<
  ...
  private modelAnchor: XRAnchor | undefined = undefined;
  private _anchorYOffset = 0;
  private _manualYOffset = 0;
  ...
  public setVerticalOffset(offset: number) {
    const delta = offset - this._manualYOffset;
    this._manualYOffset = offset;

    if (this.isPresenting && !this.modelAnchor) {
      const scene = this.presentedScene!;
      this.goalPosition.y += delta;
      scene.pivot.position.y += delta;
    }
  }
  ...
  async resolveARSession(): Promise<XRSession> {
    ...
    this.threeRenderer.xr.setReferenceSpaceType('local-floor');

    const viewerSpace = await session.requestReferenceSpace('viewer');
    const worldSpace = await session.requestReferenceSpace('local-floor');

    (this as any)._viewerSpace = viewerSpace;
    (this as any)._worldSpace = worldSpace;

    (worldSpace as any).addEventListener?.('reset', () => {
    });
  ...
  async present(scene: ModelScene, environmentEstimation: boolean = false):
    ...
    const im = (currentSession as any).interactionMode as string | undefined;
    this.xrMode = im === 'screen-space' ? XRMode.SCREEN_SPACE : XRMode.WORLD_SPACE;
    this.tracking = true;
    this.frames = 0;
  ...
  private placeInitially() {
    ...
    if (this.parsedAnchorOffset != null) {
      ...
      if (this.placementBox) this.placementBox.show = false;
    ...
    if (this.xrMode === XRMode.WORLD_SPACE) {
      ...
      if (this.placementBox) this.placementBox.show = true;
  ...
  private getHitPoint(hitResult: XRHitTestResult): Vector3 | null {
    const refSpace = (this as any)._worldSpace as XRReferenceSpace;
    ...
    if (this.placeOnWall) {
      ...
      return normalY < -0.8 ? null : hitPosition.setFromMatrixPosition(hitMatrix);
    }
 ...
 */
  public async moveToAnchor(frame: XRFrame) {
    if (this.parsedAnchorOffset != null || this.placementComplete || this.modelAnchor != null) return;
  
    const worldSpace = (this as any)._worldSpace as XRReferenceSpace;
  
    if (this.placeOnCeiling) {
      const ceilingAnchor = await this._tryCeilingPlaneAnchor(frame);
      if (ceilingAnchor) {
        this.placementComplete = true;
        if (this.placementBox) this.placementBox.show = false;
        this.presentedScene!.setShadowIntensity(AR_SHADOW_INTENSITY);
    ...
    const pt = this.getHitPoint(hit);
    if (!pt) return;
  
    const cam = this.presentedScene!.getCamera();
    const camPos = cam.getWorldPosition(new Vector3());
    if (pt.distanceTo(camPos) < MIN_WORLD_SPACE_DISTANCE) return;
    if (!this._isStableHit(pt)) return;
  
    try {
      const anchor = await this._createAnchorFromHit(frame, hit, worldSpace);
      const s = this.presentedScene!.pivot.scale.x || this.goalScale || 1;
      this._anchorYOffset = this._computeAttachYOffset(s);
      ...
      if (anchor) {
        ...
        const desired = new Vector3(
          pt.x,
          pt.y + this._anchorYOffset + this._manualYOffset,
          pt.z
        );
        this.presentedScene!.pivot.position.copy(desired);
        this.goalPosition.copy(desired);
      } else {
        const desired = new Vector3(
          pt.x,
          pt.y + this._anchorYOffset + this._manualYOffset,
  ...
  private isViewPointingUp(thresholdDeg: number = CEILING_ORIENTATION_THRESHOLD): boolean {
    ...
    const realCam: any = (cam as any).isArrayCamera && Array.isArray((cam as any).cameras)
  ...
  private handleFirstView(frame: XRFrame, time: number) {
    ...
    if (this.xrMode === XRMode.WORLD_SPACE && !this.modelAnchor) {
  ...
  public onWebXRFrame(time: number, frame: XRFrame) {
    ...
    const worldSpace = (this as any)._worldSpace as XRReferenceSpace;
    const pose = frame.getViewerPose(worldSpace);

    if (pose == null) {
      if (this.tracking === true && this.frames > INIT_FRAMES) {
        this.tracking = false;
        this.dispatchEvent({ type: 'tracking', status: 'not-tracking' });
      }
    ...
    if (this.tracking === false) {
      this.tracking = true;
      this.dispatchEvent({ type: 'tracking', status: 'tracking' });
    }

    if ((pose as any).emulatedPosition) {
      this.threeRenderer.clear();
      return;
    }

    const scene = this.presentedScene;
    if (!scene || !scene.element.loaded) {
      this.threeRenderer.clear();
      return;
    }
    ...
    for (const view of pose.views) {
      ...
      if (isFirstView) {
        ...
        if (this.modelAnchor) {
          // Read anchor and apply the stored offset so top/bottom sits on the plane
          this._applyAnchorPoseWithOffset(frame, worldSpace);
        } else {
  ...
  */
  private calculateWorldSpaceOptimalPlacement(scene: ModelScene, camera: Camera): {position: Vector3, scale: number} {
    ...
    const yOffset = (this.placeOnCeiling ? +1 : -1) * (optimalScale * halfExtent.y);
    const finalPosition = idealWorldPosition.clone().add(new Vector3(0, yOffset + this._manualYOffset, 0));
  ...
  private _stableHits: Vector3[] = [];
  private _stableNeeded = 10;
  private _stableEpsilon = 0.02;

  private _isStableHit(pt: Vector3): boolean {
    this._stableHits.push(pt.clone());
    if (this._stableHits.length > this._stableNeeded) this._stableHits.shift();
    if (this._stableHits.length < this._stableNeeded) return false;

    // Check spread
    const first = this._stableHits[0];
    for (let i = 1; i < this._stableHits.length; i++) {
      if (this._stableHits[i].distanceTo(first) > this._stableEpsilon) return false;
    }
    return true;
  }

  private async _createAnchorFromHit(
    frame: XRFrame,
    hit: XRHitTestResult,
    worldSpace: XRReferenceSpace
  ): Promise<XRAnchor | undefined> {
    const pose = hit.getPose(worldSpace);
    if (!pose) return undefined;

    if ((frame as any).createAnchor) {
      return await (frame as any).createAnchor(pose.transform, worldSpace) as XRAnchor;
    }

    if ((hit as any).createAnchor) {
      return await (hit as any).createAnchor(pose.transform) as XRAnchor;
    }

    return undefined;
  }
  private async _tryCeilingPlaneAnchor(frame: XRFrame): Promise<XRAnchor | undefined> {
    const planes: any = (frame as any).worldInformation?.detectedPlanes; // correct accessor
    if (!planes || !planes.size) return undefined;
    ...
    for (const plane of planes) {
      const orientation = plane.orientation as ('horizontal' | 'vertical' | undefined);
      if (orientation !== 'horizontal') continue;
  
      const centerPose = frame.getPose(plane.planeSpace, worldSpace);
      if (!centerPose) continue;
  
      const m = new Matrix4().fromArray(centerPose.transform.matrix);
      const planePos = new Vector3().setFromMatrixPosition(m);
      if (planePos.y <= camPos.y + 0.5) continue;
  
      const label: string | undefined = plane.semanticLabel || plane.semanticLabels?.[0];
      if (label && !/ceiling/i.test(label)) continue;
  
      const offset = new XRRigidTransform({x: 0, y: 0, z: 0});
      let anchor: XRAnchor | undefined;
      if ((frame as any).createAnchor) {
        anchor = await (frame as any).createAnchor(offset, plane.planeSpace) as XRAnchor;
      } else {
        anchor = undefined;
      }
      ...
      if (anchor) {
        ...
        const desired = new Vector3(
          planePos.x,
          planePos.y + this._anchorYOffset + this._manualYOffset,
          planePos.z
        );
        this.presentedScene!.pivot.position.copy(desired);
        this.goalPosition.copy(desired);
  ...
  private _computeAttachYOffset(scale: number): number {
    const box = this.presentedScene!.boundingBox;
    if (this.placeOnCeiling) {
      return -box.max.y * scale;
    }
    if (this.placeOnWall) {
      return 0;
    }
  ...
  private _applyAnchorPoseWithOffset(frame: XRFrame, worldSpace: XRReferenceSpace) {
    if (!this.modelAnchor) return;
  
    const scene = this._presentedScene!;
    const anchorPose = frame.getPose(this.modelAnchor.anchorSpace, worldSpace);
    if (!anchorPose) return;
  
    const m = new Matrix4().fromArray(anchorPose.transform.matrix);
    const anchorPos = new Vector3().setFromMatrixPosition(m);
  
    const s = scene.pivot.scale.x || this.goalScale || 1;
    if (this._anchorYOffset === 0) {
      this._anchorYOffset = this._computeAttachYOffset(s);
    }
  
    const finalY = anchorPos.y + this._anchorYOffset + this._manualYOffset;
    scene.pivot.position.set(anchorPos.x, finalY, anchorPos.z);
    this.goalPosition.copy(scene.pivot.position);
  }
}
